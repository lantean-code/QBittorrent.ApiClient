...
namespace QBittorrent.ApiClient.Test
{
    public sealed class HttpClientExtensionsTests : IDisposable
    {
        ...
        public async Task GIVEN_FormUrlEncodedBuilder_WHEN_PostAsyncCalled_THEN_ShouldPostEncodedPayload()
        {
            ...
            var response = await _target.PostAsync("/api/v2/test", builder, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task GIVEN_QueryBuilder_WHEN_GetAsyncCalled_THEN_ShouldAppendQueryStringToUrl()
        {
            ...
            var response = await _target.GetAsync("/api/v2/test", builder, cancellationToken: TestContext.Current.CancellationToken);
